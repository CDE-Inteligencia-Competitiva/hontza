<?php
function diccionario_traer_alerta_callback(){    
    //return 'Funcion desactivada';
    diccionario_traer_alerta_local_sources();
    return date('Y-m-d H:i');
}
function diccionario_traer_alerta_local_sources(){
    db_set_active('hontza_alerta');
    $sources_array=diccionario_alerta_get_sources_array();
    db_set_active();
    $traer_array=array();
    $kont=0;
    if(!empty($sources_array)){
        foreach($sources_array as $i=>$row){
            ...
            if(isset($r->lid) && !empty($r->lid)){            
                ...
                db_set_active('hontza_alerta');
                $traducciones=array();
                $is_traducido=diccionario_alerta_is_traducido($row,$traducciones);
                db_set_active();
                if($is_traducido){
                    /*print '####################INI##########################<BR>';
                    print $row->source.'<BR>';
    ...
    exit();
    db_set_active();
    //$output=diccionario_traer_guardar_traer_array($traer_array);
    return $output;
}
function diccionario_alerta_get_sources_array(){
    $result=array();
...
function diccionario_alerta_is_traducido($row,&$target_array){
    if(diccionario_alerta_contiene_php($row->source)){
        return 0;
    }
    if(diccionario_alerta_contiene_curl_error($row->source)){
        return 0;
    }
    if(diccionario_alerta_contiene_custom_css($row->source)){
        return 0;
    }
    if(diccionario_alerta_contiene($row->source)){
        return 0;
    }
    //
    $target_array=diccionario_alerta_get_target_array($row->lid);
    if(count($target_array)>0){
        ...
        foreach($target_array as $i=>$r){
            ...
            //
            if(in_array($row->source,array('Jan 21','Jan 25'))){
                return 0;
            }
            //
...
function diccionario_alerta_get_target_array($lid){
    $result=array();
    //print 'lid===='.$lid.'BR>';
...
function diccionario_alerta_contiene($source){
    $v=strtolower($source);
    //intelsat-2016
...
function diccionario_alerta_contiene_php($source){
    $v=strtolower($source);
    $find='<?php';
    $pos=strpos($v,$find);
    if($pos===FALSE){
...
function diccionario_alerta_contiene_curl_error($source){
    $v=strtolower($source);
    $find='curl error';
    $pos=strpos($v,$find);
    if($pos===FALSE){
...
function diccionario_alerta_contiene_custom_css($source){
    $v=strtolower($source);
    $find='custom.css';
    $pos=strpos($v,$find);
    if($pos===FALSE){
...
function diccionario_traer_guardar_traer_array($traer_array){
    $html=array();
...
function diccionario_alerta_get_contiene(){
    if(defined('_DICCIONARIO_CONTIENE_NET')){
        return _DICCIONARIO_CONTIENE_NET;
    }
    return '';
}