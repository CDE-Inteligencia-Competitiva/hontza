<?php
function contenidos_reports_custom_access(){
...
function contenidos_reports_callback(){
    global $user;
    //
    $headers=array();
   ...
   $filter_fields=contenidos_reports_define_filter_fields();
    
   $where=array();
   $where[]='1';
   $where[]='users.uid = '.$user->uid;
   $where[]='node.type in ("my_report")';  
   
   if(!empty($filter_fields)){
       foreach($filter_fields as $k=>$f){
           $v=hontza_get_gestion_usuarios_filter_value($f,'contenidos_reports_filtro');
           if(!empty($v)){
                switch($f){
                    case 'grupo_nid':
                        //intelsat-2016
    ...
    $where[]=hontza_get_usuarios_acceso_where_time('node.changed','contenidos_reports_filtro','fecha_inicio','fecha_fin');
    ...
    while ($r = db_fetch_object($res)) {
      ...
      $rows[$kont][4]=array('data'=>contenidos_reports_define_acciones($r),'style'=>'white-space:nowrap;');
    ...
    drupal_set_title(t('My Reports'));
    /*$output=contenidos_my_menu().$output;
    return $output;*/
    return contenidos_reports_define_header().drupal_get_form('contenidos_reports_bulk_form',array($output));
}
function contenidos_reports_define_acciones($row){
    return contenidos_define_acciones($row,drupal_get_destination());
}
function contenidos_reports_define_header(){
    my_add_buscar_js();
    return drupal_get_form('contenidos_reports_filtro_form');
}
function contenidos_reports_bulk_form(){
    $form=array();
    $vars=func_get_args();
    //
...
function contenidos_reports_bulk_form_submit($form, &$form_state) {
    contenidos_reports_bulk_submit($form,$form_state,'contenidos_reports');
}
function contenidos_reports_delete_reports_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('delete_node','contenidos_reports_nid_array','contenidos/reports');
    return $form;
}
function contenidos_reports_delete_reports_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url=$base_url.'/contenidos/reports';
    call_bulk_confirm_form_submit($form,$form_state,'delete_node',$url,true);
}
function contenidos_reports_publish_post_reports_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('publish_post','contenidos_reports_nid_array','contenidos/reports');
    return $form;
}
function contenidos_reports_unpublish_post_reports_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('unpublish_post','contenidos_reports_nid_array','contenidos/reports');
    return $form;
}
function contenidos_reports_publish_post_reports_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url='contenidos/reports';
    call_bulk_confirm_form_submit($form,$form_state,'publish_post',$url,true);    
}
function contenidos_reports_unpublish_post_reports_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url='contenidos/reports';
    call_bulk_confirm_form_submit($form,$form_state,'unpublish_post',$url,true);    
}
function contenidos_reports_filtro_form(){
    $fs_title=t('Search');
    if(contenidos_reports_is_filter_activated()){
    ...
    /*$form['file_buscar_fs']['node_type']=array(
        ...
        '#options'=>contenidos_reports_define_node_type_options(),
        '#default_value'=>contenidos_reports_get_filter_value('node_type'),
    );*/
    contenidos_add_filter_form_fields('contenidos_reports_filtro',$form);
...
function contenidos_reports_bulk_submit($form,$form_state,$type=''){
    ...
    if(in_array($button_name,array('delete_node_btn','publish_post_btn','unpublish_post_btn'))){
        if(isset($button['#post']['txek_nid']) && !empty($button['#post']['txek_nid'])){
            ...
            $_SESSION['contenidos_reports_nid_array']=$nid_array;                
            if(strcmp($button_name,'delete_node_btn')==0){
                if($type=='contenidos_reports'){
                    drupal_goto('contenidos/reports/delete_reports_confirm');
                }                   
            }else if(strcmp($button_name,'publish_post_btn')==0){                
                if($type=='contenidos_reports'){
                    drupal_goto('contenidos/reports/publish_post_reports_confirm');
                }              
            }else if(strcmp($button_name,'unpublish_post_btn')==0){
                if($type=='contenidos_reports'){
                    drupal_goto('contenidos/reports/unpublish_post_reports_confirm');
                }             
            }
        }
    }
}
function contenidos_reports_filtro_form_submit(&$form, &$form_state){
    if(isset($form_state['clicked_button']) && !empty($form_state['clicked_button'])){
        $name=$form_state['clicked_button']['#name'];
        $key='contenidos_reports_filtro';
        if(strcmp($name,'limpiar')==0){
            ...
            $fields=contenidos_reports_define_filter_fields();
            if(count($fields)>0){
...
function contenidos_reports_is_filter_activated(){
    $fields=contenidos_fuentes_define_filter_fields();
    if(count($fields)>0){
        foreach($fields as $i=>$f){
            if(isset($_SESSION['contenidos_reports_filtro']['filter'][$f]) && !empty($_SESSION['contenidos_reports_filtro']['filter'][$f])){
                return 1;
            }
        }
    }
...
function contenidos_reports_define_filter_fields(){
