...
function contenidos_collaboration_callback(){
    ...
    if(!empty($filter_fields)){
       foreach($filter_fields as $k=>$f){
           $v=hontza_get_gestion_usuarios_filter_value($f,'contenidos_collaboration_filtro');
           if(!empty($v)){
                switch($f){
                    case 'grupo_nid':
                        //intelsat-2016
    ...
    $where[]=hontza_get_usuarios_acceso_where_time('node.changed','contenidos_collaboration_filtro','fecha_inicio','fecha_fin');
    ...
    while ($r = db_fetch_object($res)) {
      ...
      $last_update=contenidos_collaboration_get_last_update($r->nid,$r->node_changed);
      $rows[$kont][4]=$last_update;
      $rows[$kont][5]=array('data'=>contenidos_collaboration_define_acciones($r),'style'=>'white-space:nowrap;');
    ...
    return $output;*/
    return contenidos_collaboration_header().drupal_get_form('contenidos_collaboration_bulk_form',array($output));
}
function contenidos_collaboration_define_acciones($r){
    return contenidos_define_acciones($r,'destination=contenidos/collaboration');
}
function contenidos_collaboration_get_last_update($nid,$node_changed){
    //return contenidos_debates_get_last_update($nid,$node_changed);
    return date('Y-m-d H:i',$node_changed);
}
function contenidos_collaboration_header(){
    my_add_buscar_js();
    return drupal_get_form('contenidos_collaboration_filtro_form');
}
function contenidos_collaboration_bulk_form(){
    $form=array();
    $vars=func_get_args();
    //
...
function contenidos_collaboration_bulk_form_submit($form, &$form_state) {
    panel_admin_collaboration_gestion_collaboration_bulk_submit($form,$form_state,'contenidos_collaboration');
}
function contenidos_collaboration_delete_collaboration_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('delete_node','block_panel_admin_collaboration_nid_array','contenidos/collaboration');
    return $form;
}
function contenidos_collaboration_delete_collaboration_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url=$base_url.'/contenidos/collaboration';
    call_bulk_confirm_form_submit($form,$form_state,'delete_node',$url,true);
}
function contenidos_collaboration_publish_post_collaboration_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('publish_post','block_panel_admin_collaboration_nid_array','contenidos/collaboration');
    return $form;
}
function contenidos_collaboration_unpublish_post_collaboration_confirm_form(){
    $form=panel_admin_grupos_get_grupos_confirm('unpublish_post','block_panel_admin_collaboration_nid_array','contenidos/collaboration');
    return $form;
}
function contenidos_collaboration_publish_post_collaboration_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url='contenidos/collaboration';
    call_bulk_confirm_form_submit($form,$form_state,'publish_post',$url,true);    
}
function contenidos_collaboration_unpublish_post_collaboration_confirm_form_submit(&$form, &$form_state){
    global $base_url;
    $url='contenidos/collaboration';
    call_bulk_confirm_form_submit($form,$form_state,'unpublish_post',$url,true);    
}
function contenidos_collaboration_filtro_form(){
    $fs_title=t('Search');
    if(contenidos_collaboration_is_filter_activated()){
    ...
    /*$form['file_buscar_fs']['node_type']=array(
        ...
        '#options'=>contenidos_collaboration_define_node_type_options(),
        '#default_value'=>contenidos_collaboration_get_filter_value('node_type'),
    );*/
    contenidos_add_filter_form_fields('contenidos_collaboration_filtro',$form);
...
function contenidos_collaboration_filtro_form_submit(&$form, &$form_state){
    if(isset($form_state['clicked_button']) && !empty($form_state['clicked_button'])){
        $name=$form_state['clicked_button']['#name'];
        $key='contenidos_collaboration_filtro';
        if(strcmp($name,'limpiar')==0){
            ...
            $fields=contenidos_collaboration_define_filter_fields();
            if(count($fields)>0){
...
function contenidos_collaboration_is_filter_activated(){
    $fields=contenidos_fuentes_define_filter_fields();
    if(count($fields)>0){
        foreach($fields as $i=>$f){
            if(isset($_SESSION['contenidos_collaboration_filtro']['filter'][$f]) && !empty($_SESSION['contenidos_collaboration_filtro']['filter'][$f])){
                return 1;
            }
        }
    }
...
function contenidos_collaboration_define_filter_fields(){
