<?php
function red_local_canales_local_callback(){
   $where=array();
   $where[]="1";
   //
   ...
   if(!empty($my_grupo) && isset($my_grupo->nid) && !empty($my_grupo->nid)){
    ...
    FROM {node} n 
    LEFT JOIN {red_compartir_canal} c ON n.vid=c.vid
...
function red_local_canal_get_estrellas_average_row($nid){
    return hontza_get_avg_rating($nid);
}
function red_local_canal_local_define_acciones($r){
    $html=array();
